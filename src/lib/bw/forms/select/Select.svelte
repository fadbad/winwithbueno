<script lang='ts'>
    ...
    import _ClearIcon from './icons/ClearIcon.svelte';
    ...
    export let inputAttributes: any = {};
    ...
    export let wrapperClasses = 'mb-6';
    ...
    function addCreatableItem(_items, _filterText) {
        ...
        const itemToCreate: any = createItem(_filterText);
    ...
    const updateValueText = v => {
        if(typeof v === 'string'){
            const i = items.find(x => x[optionIdentifier] === v)
            value = i || v
        }
    }
...
<style>
    .selectContainer {
        ...
        border: var(--border, 2px solid rgb(203 213 225));
...
{#if label}
    <div class="text-md text-slate-600">{label}</div>
{/if}
...
<div
    ...
    {#if !isMulti && showSelectedItem}
        <div class="selectedItem" style={selectedItemStyles} on:focus={handleFocus}>
            <svelte:component
                this={Selection}
                item={value}
                {getSelectionLabel} />
    ...
    {#if !showClearIcon && (showIndicator || (showChevron && !value) || (!isSearchable && !isDisabled && !isWaiting && ((showSelectedItem && !isClearable) || !showSelectedItem)))}
        <div class="indicator transition-all ease-in" aria-hidden="true" class:flipped={listOpen}>
            {#if indicatorSvg}
                {@html indicatorSvg}
            {:else}
                <svg
...
{#if help}
    <div class="text-sm text-slate-400 mt-0">{help}</div>
{/if}
</div>
